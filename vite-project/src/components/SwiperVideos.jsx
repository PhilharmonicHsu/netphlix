...
import {getMovieGenreById} from "./utils.js";
...
export default function SwiperVideos({children, videos, category}) {
  ...
  const handleCardClick = async (movie, event) => {
          ...
          <div className={getClasses(index)}>
            ...
            <div className="simple_info">
              <div className="buttons">
                ...
                <button className="info_btn" onClick={(event) => handleCardClick(video, event)}>
                  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
              ...
              <div className="category">
                {
                  video.genre_ids.slice(0, 3).map(genre_id => getMovieGenreById(genre_id)).join('。')
                }
              </div>
            </div>
          </div>
          <dialog
            ref={dialogRef} id="example-dialog"
            style={{
              animation: isDialogOpen ? "scale-up 3s ease forwards" : "",
            }}
          >
            <div>
              <h1>Dialog Title</h1>
